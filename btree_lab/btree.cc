      ...
      if (testkey == key)
      {
        ...
        else
        {
          ...
          if (op == BTREE_OP_UPDATE)
          {
            ...
            if (setValErr != ERROR_NOERROR)
            {
              return setValErr;
            }
            ERROR_T serializeBErr = b.Serialize(buffercache, node);
            if (serializeBErr != ERROR_NOERROR)
            {
              return serializeBErr;
            }
            return ERROR_NOERROR;
          }
          return ERROR_UNIMPL;
...
ERROR_T BTreeIndex::Insert(const KEY_T &key, const VALUE_T &value)
{
  ...
  KEY_T new_key;
  KEY_T key1 = key;
  return InsertInternal(superblock.info.rootnode, key1, valueparam, new_node, new_key);
  // return ERROR_UNIMPL;
}
...
ERROR_T BTreeIndex::Split(const SIZE_T &node, SIZE_T &new_node, KEY_T &new_key)
{
  ...
  for (iter = 0; iter <= b.info.numkeys; iter++)
  {
    if (iter > split_ind)
    {
      ...
      rhs.info.numkeys++;
      if (rc = b.GetPtr(iter, temp_ptr))
        return rc;
      if (rc = b.GetKey(iter, temp_key))
        return rc;
      // zero lhs
      if (rc = b.SetPtr(iter, s))
        return rc;
      if (rc = b.SetKey(iter, z))
        return rc;
      // set stuff on rhs
      if (rc = rhs.SetPtr(iter - split_ind, temp_ptr))
        return rc;
      if (rc = rhs.SetKey(iter - split_ind, temp_key))
        return rc;
      shift_count++;
    }
  }
...
ERROR_T BTreeIndex::split_leaf(const SIZE_T &node, SIZE_T &new_node, KEY_T &new_key)
{
  ...
  for (iter = 0; iter <= b.info.numkeys; iter++)
  {
    if (iter > split_ind)
    {
      rhs.info.numkeys++;
      // get stuff to move over
      if (rc = b.GetVal(iter, temp_val))
        return rc;
      if (rc = b.GetKey(iter, temp_key))
        return rc;
      // zero lhs
      if (rc = b.SetVal(iter, z))
        return rc;
      if (rc = b.SetKey(iter, k))
        return rc;
      // set stuff on rhs
      if (rc = rhs.SetVal(iter - split_ind, temp_val))
        return rc;
      if (rc = rhs.SetKey(iter - split_ind, temp_key))
        return rc;
      shift_count++;
                                   ...
                                   KEY_T &new_key)
{
  ...
  case BTREE_INTERIOR_NODE:
    for (offset = 0; offset < b.info.numkeys; offset++)
    {
      ...
      {
        if (rc = b.GetPtr(offset, temp_ptr))
        {
          return rc;
        }
        if (rc = InsertInternal(temp_ptr, key, value, new_node, new_key))
        {
          return rc;
        }
        ...
        else if (new_node && b.info.numkeys < b.info.GetNumSlotsAsInterior())
        {
          ...
          for (int shift_ind = offset; shift_ind < b.info.numkeys; shift_ind++)
          {
            if (rc = b.GetPtr(shift_ind, temp_ptr))
              return rc;
            if (rc = b.GetKey(shift_ind, temp_key))
              return rc;

            if (rc = b.SetPtr(shift_ind + 1, temp_ptr))
              return rc;
            if (rc = b.SetKey(shift_ind + 1, temp_key))
              return rc;
          }
          if (rc = b.SetPtr(offset, new_node))
            return rc;
          if (rc = b.SetKey(offset, new_key))
            return rc;
        ...
        else if (new_node && b.info.numkeys == b.info.GetNumSlotsAsInterior())
        {
          ...
          else
          {
            ...
            for (int offset = 0; offset < rhs.info.numkeys; offset++)
            {
              if (rc = rhs.GetKey(offset, temp_key))
                return rc;
              if (rc = rhs.GetPtr(offset, temp_ptr))
                return rc;
          ...
          if (b.info.nodetype == BTREE_ROOT_NODE)
          {
            ...
            if (rc = AllocateNode(new_root_block))
              return rc;
            if (rc = AllocateNode(new_node))
              return rc;

            if (rc = new_root.SetKey(0, new_key))
            {
              return rc;
            }
            if (rc = new_root.SetPtr(0, node))
            {
              return rc;
            }
            if (rc = new_root.SetPtr(1, new_node))
            {
              return rc;
            }
            ...
            if (rc = superblock.Serialize(buffercache, superblock_index))
              return rc;
