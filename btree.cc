...
ERROR_T BTreeIndex::FindNode(const SIZE_T &node,
 const BTreeOp op,
 const KEY_T &key,
 VALUE_T &value)
{
  ...
  // this one, if it exists
        rc=b.GetPtr(offset,ptr);
        if (rc) { return rc; }
        return FindNode(ptr,op,key,value);
      }
    }
    // if we got here, we need to go to the next pointer, if it exists
    if (b.info.numkeys>0) { 
        ...
        if (op==BTREE_OP_INSERT) { 
          ...
          if (b.info.numkeys < b.info.keysize) {
            ...
            // for loop start at the end
              ...
              // shifto > offset, shifto--
              // b.setPtr(shifto + 1, b.getPtr(shifto, ptr))
              // also setkey + setval

            // setPtr(offset) also setkey, setval

          }
          // we have slot where we want to put it, move over everything to the right of it by one
          // else if not the root, split then insert (recursive insert on parent)
          // else if root, then split (create new root)
